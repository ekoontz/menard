...
(defn testing2 []
  (let [skeleton
    ...
    (log/info (str "HII..." (syntax-tree skeleton)))
    skeleton))

;; ^^ above is the escape from ruins of:
(defn testing []
  (let [skeleton
        (-> (first (filter #(= (:rule %) "s") grammar))
            (unify
             ...
             (comment
               ...
               ((fn [tree]
                  (first (g/add-rule tree grammar syntax-tree "nbar")))))))]
     (log/info (str "HII..." skeleton))
    (let [head-spec (u/get-in skeleton (g/frontier skeleton))
          lexeme (first (->> (flatten (vals lexicon))
                             (filter #(not (= :fail (unify % head-spec))))))]
      skeleton
      (comment
        (-> skeleton
            (u/assoc-in! [:babylon.generate/started?] true)
            ((fn [tree]
               (log/info (str "the frontier is: " (g/frontier skeleton)))
               (first (g/add-lexeme tree index-fn syntax-tree)))))))))
  ;;             (u/assoc-in! tree (g/frontier skeleton) (u/copy lexeme))))))))

  ;;          (u/assoc-in! (concat (g/frontier skeleton) [:babylon.generate/done?])
  ;;                       true))))))))
